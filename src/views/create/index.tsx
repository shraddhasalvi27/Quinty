...
export const CreateView: FC<CreateViewProps> = ({ setOpenCreateModal }) => {
  ...
  const [token, setToken] =
    useState<typeof initialTokenState>(initialTokenState);
  ...
  const createToken = useCallback(
    async (token: typeof initialTokenState) => {
      if (!publicKey) {
        notify({ type: "error", message: "Connect wallet to continue." });
        return;
      }
  ...
  const handleImageChange = async (
    ...
    if (file) {
      ...
      if (imgUrl) {
        setToken({ ...token, image: imgUrl }); // image is definitely a string here
      } else {
  ...
  const uploadMetadata = async (token: typeof initialTokenState) => {
    setIsLoading(true);
